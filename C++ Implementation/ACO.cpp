...
ACO::ACO(int nAnts, int nCities, double alpha, double beta, double q, double ro, double taumax, int initCity)
{
	NUMBEROFANTS = nAnts;
...
{
	return sqrt(pow(CITIES[cityi][0] - CITIES[cityj][0], 2) + pow(CITIES[cityi][1] - CITIES[cityj][1], 2));
}
...
{
	double ETAij = pow(1.0 / distance(cityi, cityj), BETA);
	...
	for (int c = 0; c < NUMBEROFCITIES; c++)
	{
		if (exists(cityi, c))
		{
			if (!vizited(antk, c))
			{
				double ETA = pow(1.0 / distance(cityi, c), BETA);
...
int ACO::city()
{
	...
	while (sum < xi && i < NUMBEROFCITIES - 2)
	{
		i++;
		sum += PROBS[i][0];
	}
...
void ACO::optimize(int ITERATIONS)
{
	for (int iterations = 1; iterations <= ITERATIONS; iterations++)
	{
		...
		for (int k = 0; k < NUMBEROFANTS; k++)
		{
			...
			int attempts = 0;
			while (0 != valid(k, iterations) && attempts < 100)
			{
				for (int i = 0; i < NUMBEROFCITIES; i++)
				{
					ROUTES[k][i] = -1;
			...
			if (attempts >= 100)
			{
				cout << "  :: ant " << k << " failed to find valid route!" << endl;
				continue;
			}
			...
			cout << " : ant " << k << " has ended! Length: " << rlength << endl;
		}
		...
		cout << "ITERATION " << iterations << " HAS ENDED!" << endl
				 << endl;
	}
}
...
void ACO::printGRAPH()
{
	...
	cout << endl
			 << "Number of connections: " << count << endl
			 << endl;
}
